...
## Takeaways
1. Feature engineering is the most important part for this competition. My LB jumped from 0.32 (with only ~10 features) to 0.38 because of addming more features. Besides, some of the top influential features could lead to overfitting (lower CV in the end), so it's important to be careful with them (I dropped them).
...
3. What could be further improved to achieve a higher F1 score - add more features (adding features involving the information of 'None' prediction; adding features involving text information of the product names; etc.); better feature selection schema; ensemble different models (in discussion forum, some people said LightGBM gives better performance than XGboost in this competition).
